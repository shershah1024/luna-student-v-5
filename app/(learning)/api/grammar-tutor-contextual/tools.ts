...
export function createVocabularyTools(userId: string, taskId: string) {
  return {
    ...
    grammarTable: tool({
      description: 'Create a simple grammar table to show patterns, conjugations, or declensions. RESTRICTIONS: Maximum 6 rows, maximum 3 columns to avoid overwhelming A1 students. Always assess student knowledge with questions first before using this tool.',
      inputSchema: z.object({
        title: z.string().describe('Title for the table (e.g., "German Articles", "Verb Conjugation: sein")'),
        headers: z.array(z.string()).max(3).describe('Column headers for the table (maximum 3 columns)'),
        rows: z.array(z.array(z.object({
          text: z.string().describe('Cell content'),
          color: z.string().optional().describe('Optional color code for the cell (e.g., "bg-blue-100", "bg-green-100", "bg-yellow-100", "bg-red-100")')
        }))).max(6).describe('Table rows - each row is an array of cell objects (maximum 6 rows)'),
        caption: z.string().describe('Caption explaining what this table shows (required for A1 students)'),
        notes: z.array(z.string()).optional().describe('Optional notes or explanations below the table'),
      }),
      execute: async ({ title, headers, rows, caption, notes }) => {
        console.log('[GRAMMAR TOOLS] grammarTable executed:', { title, headers, rows, caption, notes });
    ...
    languageBridge: tool({
      description: 'Compare German and English language structures to help A1 learners understand patterns and differences. Use basic grammar terms but focus on practical usage and application.',
      inputSchema: z.object({
        concept: z.string().describe('The grammar concept being compared (e.g., "possessive pronouns", "articles", "word order", "verb conjugation")'),
        germanExample: z.string().describe('German example demonstrating the concept'),
        englishExample: z.string().describe('English equivalent or similar example'),
        explanation: z.string().describe('Simple explanation focusing on how to use this concept, not what it means grammatically'),
        highlights: z.array(z.object({
          word: z.string().describe('Word or phrase to highlight'),
          language: z.enum(['german', 'english']).describe('Which language this highlight is in'),
          color: z.string().optional().describe('Optional highlight color (e.g., "bg-blue-100", "bg-green-100")')
        })).optional().describe('Words to emphasize in the comparison'),
        tip: z.string().optional().describe('Practical tip for remembering and using this pattern')
      }),
      execute: async ({ concept, germanExample, englishExample, explanation, highlights, tip }) => {
        console.log('[GRAMMAR TOOLS] languageBridge executed:', { concept, germanExample, englishExample, explanation, highlights, tip });
