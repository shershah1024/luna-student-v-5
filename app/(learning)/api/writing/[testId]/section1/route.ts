...
export async function GET(
  ...
  try {
    ...
    // Fetch section 1 writing exercise data for the specified test ID
    let query = supabase
      .from('writing_tests')
      .select('*')
      .eq('test_id', testId)
    ...
    if (error) {
      console.error('Error fetching writing section 1:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }
    
    if (!data) {
      return NextResponse.json(
        { error: 'Writing section 1 not found' },
        { status: 404 }
      )
    }
    ...
    console.error('Unexpected error in writing section 1 API:', error)
    return NextResponse.json(
