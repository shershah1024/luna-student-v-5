...
interface TelcA1Section1TextContent {
  questions: TelcA1Section1Question[];
  passage_text: string;
...
interface TelcA1Section1DatabaseData {
  main_title: string;
  text_contents: TelcA1Section1TextContent[];
  overall_instructions_text: string;
}

export async function POST(req: NextRequest) {
  try {
    console.log('[TELC-A1-SECTION-1 API] POST request received');
    ...
    console.log('[TELC-A1-SECTION-1 API] Request data:', {
      test_id,
      answers,
    ...
    console.log('[TELC-A1-SECTION-1 API] Database fetch result:', { testData, fetchError });
    ...
    let questionData: TelcA1Section1DatabaseData = testData.revised_question_data || testData.question_data;
    ...
    console.log('[TELC-A1-SECTION-1 API] Parsed question data:', questionData);
    ...
    console.log('[TELC-A1-SECTION-1 API] Processing text contents:', questionData.text_contents.length);
    
    questionData.text_contents.forEach((textContent) => {
      textContent.questions.forEach((question) => {
        // Skip example questions for scoring
        if (question.is_example) {
          console.log(`[TELC-A1-SECTION-1 API] Skipping example question ${question.question_number}`);
          return;
        }
        ...
        console.log(`[TELC-A1-SECTION-1 API] Question ${questionNumber}:`, {
          userAnswer,
          correctAnswer,
          isCorrect
        });
    ...
    console.log('[TELC-A1-SECTION-1 API] Final score:', {
      score,
      totalScore,
      percentage
    });
    ...
    console.error("Error in TELC A1 Section 1 API:", error);
    return NextResponse.json(
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("Error in TELC A1 Section 1 GET:", error);
