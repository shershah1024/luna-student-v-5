...
import { calculateScore } from "@/utils/readingScoring";
...
// Universal scoring handles all data formats - no need for specific interfaces
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Use the universal scoring function that handles all data formats
    const scoreResult = calculateScore(questionData, answers);
    
    console.log('[TELC-A1-SECTION-1 API] Score result:', scoreResult);
    ...
    if (existingScore) {
      ...
      const { data, error } = await supabase
        ...
        .update({
          ...
          exam_id: examId,
          answers: answers,
          validation_results: scoreResult.validation_results,
