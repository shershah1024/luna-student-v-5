 ...
 * Fetches all reading test data for a user:
 * - Reading test attempts with accuracy scores
...
export async function GET() {
  try {
    ...
    // Fetch reading scores (all question answers)
    const { data: readingScores, error: scoresError } = await supabase
      .from('reading_scores')
      .select('*')
      .eq('user_id', userId)
    ...
    if (scoresError) {
      console.error('Error fetching reading scores:', scoresError)
      return NextResponse.json(
        { error: 'Failed to fetch reading scores' },
        { status: 500 }
      )
    }

    if (!readingScores || readingScores.length === 0) {
      return NextResponse.json({
        stats: {
          totalTests: 0,
          totalQuestions: 0,
    ...
    // Get unique task IDs
    const taskIds = [...new Set(readingScores.map(score => score.task_id))]
    ...
    // Fetch reading task details
    const { data: readingTasks, error: readingTasksError } = await supabase
      .from('reading_tasks')
      .select('task_id, content, settings')
      .in('task_id', taskIds)

    if (readingTasksError) {
      console.error('Error fetching reading tasks:', readingTasksError)
    }

    // Create task details map
    const taskDetailsMap: Record<string, any> = {}
    tasks?.forEach(task => {
      const readingTask = readingTasks?.find(rt => rt.task_id === task.id)
      taskDetailsMap[task.id] = {
        ...task,
        content: readingTask?.content || {},
        settings: readingTask?.settings || {}
      }
    })

    // Group scores by task_id and attempt_number to create test attempts
    const attemptMap: Record<string, any[]> = {}
    readingScores.forEach(score => {
    ...
    const testAttempts = Object.entries(attemptMap).map(([key, scores]) => {
      ...
      return {
        ...
        level: taskDetails.settings?.reading_instructions?.level || taskDetails.level,
    ...
    const totalQuestions = readingScores.length
    const totalCorrect = readingScores.filter(s => s.is_correct).length
    ...
    console.error('Error in reading dashboard API:', error)
    return NextResponse.json(
