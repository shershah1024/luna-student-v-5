...
import { z } from "zod";
import { withProtection, handleApiError } from "@/lib/api-middleware";
...
export async function POST(req: NextRequest) {
  return withProtection(
    req,
    {
      rateLimitType: "aiOperations",
      circuitBreakerType: "azureOpenAI",
    },
    async () => {
      ...
      try {
        ...
        if (!question_data || !learner_response) {
          console.error("[Essay Evaluation] Validation failed", {
            ...
            learnerResponseLength: learner_response
              ? learner_response.length
              : 0,
        ...
        try {
          ...
          if (respError) {
            console.error(
              "[Essay Evaluation] Error saving response data:",
              respError,
            );
            throw new Error(
              `Failed to save response data: ${respError.message}`,
            );
          }
        } catch (dbError) {
          console.error(
            "[Essay Evaluation] Database error when saving response:",
            dbError,
          );
    ...
    ${learner_response}`;
        ...
        if (evaluation.total_score === undefined) {
          try {
            ...
            const taskCompletionScore =
              evaluation.parameter_evaluations.task_completion.score || 0;
            const communicativeDesignScore =
              evaluation.parameter_evaluations.communicative_design.score || 0;
            evaluation.total_score =
              taskCompletionScore + communicativeDesignScore;
          } catch (scoreError) {
            console.error(
              "[Essay Evaluation] Error calculating total_score:",
              scoreError,
            );
        ...
        if (!evaluation.score_breakdown) {
          try {
            ...
            const taskCompletionComment =
              evaluation.parameter_evaluations.task_completion.comment || "";
            ...
            console.error(
              "[Essay Evaluation] Error generating score_breakdown:",
              breakdownError,
            );
        ...
        try {
          const evalData = {
            ...
            evaluation_data: {
              ...
              communicative_design:
                evaluation.parameter_evaluations.communicative_design,
          ...
          if (evalError) {
            console.error(
              "[Essay Evaluation] Error updating evaluation data:",
              evalError,
            );
          }
        } catch (evalDbError) {
          console.error(
            "[Essay Evaluation] Database error when saving evaluation:",
            evalDbError,
          );
        }
        ...
        if (isSchemaError) {
          ...
          try {
            const fallbackEvaluation = {
              overall_evaluation:
                "We couldn't generate a detailed evaluation at this time. Your essay has been saved and will be evaluated soon.",
              parameter_evaluations: {
                task_completion: {
                  ...
                  examples: [],
                  final_comment:
                    "Please check back later for detailed feedback.",
                },
                communicative_design: {
                  score: 0,
                  max_score: 1,
            ...
            console.error(
              "[Essay Evaluation] Error creating fallback evaluation:",
              fallbackError,
            );
          }
        }
