...
export function useAgentControlBar(props: UseAgentControlBarProps = {}): UseAgentControlBarReturn {
  ...
  // For speaking tests, always ensure microphone and leave controls are visible
  // Only fall back to permissions if not explicitly set
  visibleControls.microphone = visibleControls.microphone ?? publishPermissions.microphone ?? true;
  visibleControls.leave = visibleControls.leave ?? true;
